<?php
/**
* @version $Header: /cvsroot/bitweaver/_bit_libertysecure/libertysecure_lib.php,v 1.22 2009/11/12 04:05:46 wjames5 Exp $
* created 2006/08/01
* @author Will <will@onnyturf.com>
* @version $Revision: 1.22 $ $Date: 2009/11/12 04:05:46 $
...
function secure_register_permissions(){
	...
	foreach( $gLibertySystem->mContentTypes as $type ){
		if ( isset($type['handler_package']) && $gBitSystem->isPackageActive( $type['handler_package'] ) && isset( $type['handler_file'] ) && isset( $type['handler_class'] ) ){
...
function secure_content_list_sql( &$pObject, $pParamHash=NULL ) {
	...
	$ret = array();
	if ( $gBitSystem->isPackageActive( 'libertysecure' ) && !$gBitUser->isAdmin() && !( isset( $pParamHash['has_comment_view_perm'] ) && $pParamHash['has_comment_view_perm'] == TRUE) ) {
